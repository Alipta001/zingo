...
export default function ResDetailMenu() {
  const dispatch = useDispatch();
  const { id } = useParams();
  // Ensure we are selecting from the 'menu' slice
  const { data: menu, loading, error } = useSelector((state) => state.menu);
  ...
  if (loading) return <div className="menu-preview-card"><p>Loading...</p></div>;
  if (error) return <p className="error-msg">{error}</p>;
  ...
  return (
    <div className="menu-preview-card">
      ...
      <div className="menu-cards">
        {menu && menu.length > 0 ? (
          menu.map((item) => (
            // Pass the item and the restaurant ID (userId)
            <MenuItemCard key={item.id} item={item} userId={id} />
          ))
        ) : (
          <p>No items found for this restaurant.</p>
        ...
        <Link href={`/pages/menuList/${id}`} className="show-all-item">
          <button>VIEW ALL {menu?.length || 0} ITEMS</button>
        </Link>
      </div>
    </div>
  );
